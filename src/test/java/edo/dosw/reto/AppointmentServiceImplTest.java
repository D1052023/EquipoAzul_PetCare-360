...
class AppointmentServiceImplTest {
    ...
    @Test
    void findByVeterinary_validId_shouldReturnAppointments() {
        String vetId = "vet1";
        List<Appointment> appointments = Arrays.asList(new Appointment(), new Appointment());
        when(repository.findByVeterinary(vetId)).thenReturn(appointments);

        List<Appointment> result = service.findByVeterinary(vetId);

        assertEquals(2, result.size());
        verify(repository).findByVeterinary(vetId);
    }

    @Test
    void findByVeterinary_nullOrBlankId_shouldThrowException() {
        ResponseStatusException ex1 = assertThrows(ResponseStatusException.class, () -> service.findByVeterinary(null));
        assertEquals("Veterinary ID cannot be null or empty.", ex1.getReason());

        ResponseStatusException ex2 = assertThrows(ResponseStatusException.class, () -> service.findByVeterinary("   "));
        assertEquals("Veterinary ID cannot be null or empty.", ex2.getReason());
    }

    @Test
    void cancel_validId_shouldDeleteAppointment() {
        String id = "app1";
    ...
    void cancel_nullOrBlankId_shouldThrowException() {
        ResponseStatusException ex1 = assertThrows(ResponseStatusException.class, () -> service.cancel(null));
        assertEquals("Appointment ID cannot be null or empty.", ex1.getReason());

        ResponseStatusException ex2 = assertThrows(ResponseStatusException.class, () -> service.cancel("   "));
        assertEquals("Appointment ID cannot be null or empty.", ex2.getReason());
    }

    @Test
    void cancel_nonExistingId_shouldThrowException() {
        String id = "app1";
        when(repository.findById(id)).thenReturn(Optional.empty());

        ResponseStatusException ex = assertThrows(ResponseStatusException.class, () -> service.cancel(id));
        assertEquals("No appointment found with id: " + id, ex.getReason());
    }
}